...
#        connected=self.channels[self.connected_pixels]
    ...
    def calc_estimate_coordinates(self, dimensions=2, return_results=False):
        if self.estimated_coordinates==None:
            ...
            if return_results:
                return estimated_coordinates, results
            else:
    ...
    def save_chan_params(self, savepath=None):
        if savepath is None:
            savepath = os.path.split(self.savepath)[0]
        np.save(os.path.join(savepath, 'chimpy_chan_params.npy'), [self.channels, self.xs, self.ys])


class StimRecording(Recording):
    
    def __init__(self, filepath, *, savepath=None, connected_threshold=10, use_counts=True):
        ...
        fid.close()
        self.amps, counts = preprocess.get_spike_amps(self.filt_traces, return_counts = True)
        if use_counts:
            self.connected_pixels = np.where(counts > 0)[0]
        else:
